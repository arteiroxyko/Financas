...
$this->Cell(0,0.6,utf8_decode("* Atenção: cmovimentos sem descrição da categoria podem ser exibidos, devido exclusão desta, da base de dados do sistema."),0,1,'L');
$this->SetFont('Arial','I',7);
...
$pdf->Cell(19,1,utf8_decode("Relatório de exclusões de cmovimentos."),0,1,'C',0);
$pdf->Ln(0.7);

$qrv=mysql_query("SELECT * FROM cexclusoes WHERE conta_mov='$conta' && usuario_mov='$usuario' ORDER By id DESC");
if (mysql_num_rows($qrv)!==0){

$qrg=mysql_query("SELECT SUM(valor_mov) as total FROM cexclusoes WHERE tipo_mov=1 && conta_mov='$conta' && usuario_mov='$usuario'");
$rowg=mysql_fetch_array($qrg);
$entradas=$rowg['total'];

$qrg=mysql_query("SELECT SUM(valor_mov) as total FROM cexclusoes WHERE tipo_mov=0 && conta_mov='$conta' && usuario_mov='$usuario'");
$rowg=mysql_fetch_array($qrg);
$saidas=$rowg['total'];
