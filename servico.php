...
if (isset($_GET['acao']) && $_GET['acao'] == 'apagar') {
    $id = $_GET['id'];
    $log=mysql_query("SELECT * FROM smovimentos WHERE id='$id'");
    ...
    mysql_query("INSERT INTO sexclusoes (id_mov_exc,tipo_mov,desc_mov,valor_mov,cat_mov,conta_mov,data_exc,usuario_mov) values ('$idmov','$tipomov','$descmov','$valormov','$catmov','$contamov','$dataexc','$usuario')");
    mysql_query("DELETE FROM smovimentos WHERE id='$id'");
    mysql_query("DELETE FROM shistorico WHERE id_mov='$id'");
    echo mysql_error();
...
if (isset($_POST['acao']) && $_POST['acao'] == 'editar_cat') {
    ...
    mysql_query("UPDATE scategorias SET nome='$nome' WHERE id='$id'");
    echo mysql_error();
...
if (isset($_GET['acao']) && $_GET['acao'] == 'apagar_cat') {
    ...
    $qr=mysql_query("SELECT c.id FROM smovimentos g, scategorias c WHERE c.id=g.cat && c.id=$id");
    if (mysql_num_rows($qr)!==0){
        header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano'] . "&cat_err=1");
        exit();
    }
    else{
    mysql_query("DELETE FROM scategorias WHERE id='$id'");
    echo mysql_error();
...
if (isset($_POST['acao']) && $_POST['acao'] == 'editar_mov') {
    ...
    $dataed = date("Ymd");
	$qred=mysql_query("SELECT * FROM smovimentos WHERE id='$id'");
...
exit();
	...
	if ($dia!=$rowed['dia']){
    mysql_query("UPDATE smovimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO shistorico (id_mov,just_id,data,conta_mov,usuario) values ('$id','1','$dataed','1','$usuario')");
    echo mysql_error();}

	if ($mes!=$rowed['mes']){
    mysql_query("UPDATE smovimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO shistorico (id_mov,just_id,data,conta_mov,usuario) values ('$id','2','$dataed','1','$usuario')");
    echo mysql_error();}
	
	if ($ano!=$rowed['ano']){
    mysql_query("UPDATE smovimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO shistorico (id_mov,just_id,data,conta_mov,usuario) values ('$id','3','$dataed','1','$usuario')");
    echo mysql_error();}
	
	if ($tipo!=$rowed['tipo']){
    mysql_query("UPDATE smovimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO shistorico (id_mov,just_id,data,conta_mov,usuario) values ('$id','4','$dataed','1','$usuario')");
    echo mysql_error();}
	
	if ($cat!=$rowed['cat']){
    mysql_query("UPDATE smovimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO shistorico (id_mov,just_id,data,conta_mov,usuario) values ('$id','5','$dataed','1','$usuario')");
    echo mysql_error();}
	
	if ($descricao!=$rowed['descricao']){
    mysql_query("UPDATE smovimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO shistorico (id_mov,just_id,data,conta_mov,usuario) values ('$id','6','$dataed','1','$usuario')");
    echo mysql_error();}
	
	if ($valor!=$rowed['valor']){
    mysql_query("UPDATE smovimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO shistorico (id_mov,just_id,data,conta_mov,usuario) values ('$id','7','$dataed','1','$usuario')");
    echo mysql_error();}
...
if (isset($_POST['acao']) && $_POST['acao'] == 2) {
    ...
    mysql_query("INSERT INTO scategorias (nome,usuario) values ('$nome','$usuario')");
    echo mysql_error();
...
exit;
	...
	$n=1;
	while ($n <= $parcelas) {
    mysql_query("INSERT INTO smovimentos (dia,mes,ano,tipo,descricao,valor,cat,conta,nparcela,parcelas,usuario) values ('$dia','$mes','$ano','$tipo','$descricao','$valor','$cat','1','$n','$parcelas','$usuario')");
    echo mysql_error();
...
exit;
	...
	if ($tipo!=0){
    mysql_query("INSERT INTO sorcamento (mes,ano,valor,conta,usuario) values ('$mes','$ano','$valor_orcamento','1','$usuario')");
    echo mysql_error();
...
exit();
	...
	if ($tipo!=0){
	mysql_query("UPDATE sorcamento SET valor='$valor_orcamento' WHERE mes='$mes' && conta='1' && usuario='$usuario'");
    echo mysql_error();
	header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano']);
    exit();
	}
    mysql_query("UPDATE sorcamento SET valor='$valor_orcamento' WHERE mes>=$mes && ano='$ano' && conta='1' && usuario='$usuario'");
    echo mysql_error();
	header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano']);
    exit();
}
...
<title id='titulo'>SERVI&Ccedil;OS Jaqueline Arteira</title>
...
<table  width="100%" align="center" cellpadding="1" cellspacing="5"   background="imagens/menubar_bg.gif" class="we">
  <tr>
    <td colspan="11"  align="center" valign="middle" ><img src="imagens/saco.png" alt="#JaquelineArteira" width="49" height="45" align="left">
       <h2><a href=index.php style="color:#FFFF00"> Movimento da Loja</a> | <a href=consignados.php style="color:#FFFF00">Consignados</a> | <a href=servico.php style="color:#FFFFFF">Servi&ccedil;os</a></h2></td>
    <td colspan="2" align="right"><a style="color:#FFF" href="?mes=<?php echo date('m')?>&ano=<?php echo date('Y')?>">Hoje:<strong> <?php echo date('d')?> de <?php echo mostraMes(date('m'))?> de <?php echo date('Y')?></strong></a>&nbsp; </td>
  </tr>
  <tr>
    <td><select onChange="location.replace('?mes=<?php echo $mes_hoje?>&ano='+this.value)">
        <?php
for ($i=2015;$i<=2020;$i++){
?>
...
for ($i=1;$i<=12;$i++){
	?>
    <td align="center" style="<?php if ($i!=12) echo "border-right:1px solid #FFFFFF;"?> padding-right:5px"><a href="?mes=<?php echo $i?>&ano=<?php echo $ano_hoje?>" style="
    <?php if($mes_hoje==$i){?>    
    color:#448ED3; font-size:16px; font-weight:bold; background-color:#FFFFFF; padding:5px
    <?php }else{?>
    color:#FFFFFF;  font-size:16px;
    <?php }?>
    "> <?php echo mostraMes($i);?> </a></td>
    <?php
}
?>
  </tr>
...
$n_acesso=$n+1;
?>
    <td><b><font color="#035D81" size=1>
      <?php if ($n>0) echo "Ultimo acesso: ".date('d/m/Y H:i:s', strtotime($rowvisita['ultimavisita'])); else echo ""?>
      </font> <font color="#035D81" size=1><?php echo"Acesso N&ordm;: "?>
      <?php if ($n=0) echo "1"; else echo"$n_acesso" ?>
      </font></td>
    <td><font color="#035D81" size=3><?php echo $mensagem?><?php echo " <img src='imagens/xyko.png' width='25' height='25'> "?><?php echo $_SESSION['usuarioNome'];?><?php echo " "?><?php echo $_SESSION['usuarioSobrenome'];?>.</font></td>
    <td align="right" style="font-size:13px; color:rgba(4, 45, 191, 1)"><a href="javascript:;" style="font-size:12px; color:rgba(4, 45, 191, 1)" onClick="abreFecha('orcamento')"><img src="imagens/orca.png" width="16" height="16"> Or&ccedil;amento </a> <a href="javascript:;" style="font-size:12px; color:rgba(4, 45, 191, 1)" onClick="abreFecha('alterar_senha')"><img src="imagens/senha.png" width="16" height="16"> Alterar senha </a> <a href="logout.php" style="font-size:12px; color:rgba(4, 45, 191, 1)"><img src="imagens/sair.png" width="16" height="16"><?php echo "  Sair "?></a></td>
  </tr>
  <tr>
    <td></td>
  </tr>
...
<table cellpadding="5" cellspacing="0" width="1000" align="center" >
  <tr style="display:none; background-color:#ffffff" id="alterar_senha">
    <td align="left" class="welll"><form id="formulario" method="post" action="cadastro.php">
        <input type="hidden" name="acao" value="alterar_senha" />
        <input type="hidden" name="pagina" value="index.php" />
        <input type="hidden" name="usuario" value="<?php echo $usuario?>" />
        <b>Nova senha:</b>
        <input type="password" name="novasenha" id="novasenha" onKeyUp="passwordStrength(this.value)">
        <b>Confirmar nova senha:</b>
        <input type="password" name="novasenhaconf" id="novasenhaconf">
        <br>
        ...
        <p align="right">
          <input type="submit" class="btn-warning" value="Alterar" />
        </p>
      </form></td>
  </tr>
</table>
<table cellpadding="5" cellspacing="0" width="1000" align="center">
  <?php
$qr=mysql_query("SELECT SUM(valor) as total FROM sorcamento WHERE mes='$mes_hoje' && ano='$ano_hoje' && conta=1 && usuario='$usuario'");
$row=mysql_fetch_array($qr);
$total=$row['total'];

$qr=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$row=mysql_fetch_array($qr);
$gasto=$row['total'];
$resta=$total-$gasto;
...
$qra=mysql_query("SELECT SUM(valor) as total FROM sorcamento WHERE ano='$ano_hoje' && conta=1 && usuario='$usuario'");
$rowa=mysql_fetch_array($qra);
$total_ano=$rowa['total'];

$qra=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && ano='$ano_hoje'");
$rowa=mysql_fetch_array($qra);
...
$qrc=mysql_query("SELECT SUM(valor) as total FROM sorcamento WHERE mes='$mes_hoje' && ano='$ano_hoje' && conta>1 && usuario='$usuario'");
$rowc=mysql_fetch_array($qrc);
$total_cart=$rowc['total'];

$qrc=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=0 && conta>1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$rowc=mysql_fetch_array($qrc);
...
?>
  <tr style="display:none;" id="orcamento" class="welll">
    <td align="left" style="font-size:14px; color:rgb(0, 0, 0)" class="welll"><a href="javascript:;" style="font-size:12px" onClick="abreFecha('lancar_orcamento')" title="Gereciar orçamento" >Or&ccedil;amento mensal: <?php echo formata_dinheiro($gasto)?><?php echo " de "?><?php echo formata_dinheiro($total)?><?php echo " resta "?><font color="<?php if ($resta <= 0) echo "#C00"?>"><?php echo formata_dinheiro($resta)?></font>.</a><br>
      <style type="text/CSS">
.outter{
	height:20px;
	width:1000px;
...
.inner3{
	...
	filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#f61900', endColorstr='#f61900',GradientType=0 ); /* IE6-9 */"?>
      ...
      <div class="welll">
        <div class="inner">
          <center>
            <?php if ($percento <=0) echo ""; else echo $percento ?>
            %
          </center>
        </div>
      </div>
      <a href="javascript:;" style="font-size:12px" onClick="abreFecha('orcamento_anual')" title="Exibir orçamento anual">[ Exibir or&ccedil;amento anual ]</a> <a href="javascript:;" style="font-size:12px" onClick="abreFecha('orcamento_cartoes')" title="Exib. orçamento mensal serviços">[ Exib. or&ccedil;amento servi&ccedil;os ]</a></td>
  </tr>
  <tr style="display:none; background-color:#ffffff" id="lancar_orcamento">
    <td class="welll"><a href="javascript:;" style="font-size:12px; color:rgba(4, 45, 191, 1)" onClick="abreFecha('cad_orcamento')" title="Cadastre apenas uma vez."> [ Cad. Or&ccedil;amento ]</a> <a href="javascript:;" style="font-size:12px; color:rgba(4, 45, 191, 1)" onClick="abreFecha('ed_orcamento')" title="Edite um ou mais meses"> [ Editar Or&ccedil;amento ]</a>
      </dt>
  </tr>
  <tr style="display:none; background-color:#ffffff" id="cad_orcamento">
    <td class="welll"><form method="post" action="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>">
  ...
  <tr style="display:none; background-color:#ffffff" id="ed_orcamento">
    <td class="welll"><form method="post" action="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>">
        ...
        Novo valor: R$
        <input type=text value="<?php echo $total?>" name=valor id="edorcamento" length=15 onKeyPress="return(FormataReais(this,'.',',',event))">
        &nbsp;|&nbsp;
        <input type="radio" name="tipo" value="1" checked />
        Somente este &nbsp;
        <input type="radio" name="tipo" value="0" />
  ...
  <tr style="display:none;" id="orcamento_anual">
    <td><a href="javascript:;" style="font-size:12px" title="Orçamento anual">Or&ccedil;amento anual: <?php echo formata_dinheiro($gasto_ano)?><?php echo " de "?><?php echo formata_dinheiro($total_ano)?><?php echo " resta "?><font color="<?php if ($resta_ano <= 0) echo "#C00"?>"><?php echo formata_dinheiro($resta_ano)?></font>.</a><br>
      <div class="welll">
        <div class="inner2">
          <center>
            <?php if ($percento_ano <=0) echo ""; else echo $percento_ano ?>
            %
          </center>
        </div>
      </div>
  ...
  <tr style="display:none;" id="orcamento_cartoes">
    <td><a href="javascript:;" style="font-size:12px" title="Orçamento mensal serviços">Or&ccedil;amento mensal servi&ccedil;os: <?php echo formata_dinheiro($gasto_cart)?><?php echo " de "?><?php echo formata_dinheiro($total_cart)?><?php echo " resta "?><font color="<?php if ($resta_cart <= 0) echo "#C00"?>"><?php echo formata_dinheiro($resta_cart)?></font>.</a><br>
      <div class="welll">
        <div class="inner3">
          <center>
            <?php if ($percento_cart <=0) echo ""; else echo $percento_cart ?>
            %
          </center>
        </div>
      </div>
...
<table cellpadding="10" cellspacing="0" width="1000" align="center" >
  <tr>
    <td colspan="2"><h2><?php echo mostraMes($mes_hoje)?>/<?php echo $ano_hoje?></h2></td>
    <td align="right"><a href="javascript:;" onClick="abreFecha('add_cat')" class="btn"><img src="imagens/add.png" width="16" height="16"> Adicionar Categoria</a> <a href="javascript:;" onClick="abreFecha('add_movimento')" class="btn"><strong><img src="imagens/add.png" width="16" height="16"> Adicionar Movimento</strong></a></td>
  </tr>
  <tr >
    <td colspan="3" ><?php
if (isset($_GET['cat_err']) && $_GET['cat_err']==1){
?>
      <div style="padding:5px; background-color:#FF6; text-align:center; color:#030"> <strong>Esta categoria n&atilde;o pode ser removida, pois há smovimentos associados a mesma</strong> </div>
      <?php }?>
      <?php
if (isset($_GET['cat_ok']) && $_GET['cat_ok']==2){
?>
      <div style="padding:5px; background-color:#9CC; text-align:center; color:#000"> <strong>Categoria removida com sucesso!</strong> </div>
      <?php }?>
      <?php
if (isset($_GET['cat_ok']) && $_GET['cat_ok']==1){
?>
      <div style="padding:5px; background-color:#9CC; text-align:center; color:#000"> <strong>Categoria Cadastrada com sucesso!</strong> </div>
      <?php }?>
      <?php
if (isset($_GET['cat_ok']) && $_GET['cat_ok']==3){
?>
      <div style="padding:5px; background-color:#9CC; text-align:center; color:#000"> <strong>Categoria alterada com sucesso!</strong> </div>
      <?php }?>
      <?php
if (isset($_GET['ok']) && $_GET['ok']==1){
?>
      <div style="padding:5px; background-color:#9CC; text-align:center; color:#000"> <strong>Movimento Cadastrado com sucesso!</strong> </div>
      <?php }?>
      <?php
if (isset($_GET['ok']) && $_GET['ok']==2){
?>
      <div style="padding:5px; background-color:#900; text-align:center; color:#FFF"> <strong>Movimento removido com sucesso!</strong> </div>
      <?php }?>
      <?php
if (isset($_GET['ok']) && $_GET['ok']==3){
?>
      <div style="padding:5px; background-color:#9CC; text-align:center; color:#000"> <strong>Movimento alterado com sucesso!</strong> </div>
      <?php }?>
      <div style=" background-color:#F1F1F1; padding:30px;  margin:5px; display:none" id="add_cat" class="welll">
        ...
        <table width="90%">
          <tr>
            <td valign="top"><form method="post" action="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>">
            ...
            <td valign="top" align="right"><b>Editar/Remover Categorias:</b><br/>
              <br/>
              <?php
$qr=mysql_query("SELECT id, nome FROM scategorias where usuario='$usuario' ORDER BY nome");
while ($row=mysql_fetch_array($qr)){
?>
              <div id="editar2_cat_<?php echo $row['id']?>"> <?php echo $row['nome']?> <a style="font-size:10px; color:#666" onClick="return confirm('Tem certeza que deseja remover esta categoria?\nAtenção: Apenas scategorias sem smovimentos associados poderão ser removidas.')" href="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>&acao=apagar_cat&id=<?php echo $row['id']?>" title="Remover" class="btn-mini"><img src="imagens/delete.png" width="16" height="16"> remover </a> <a href="javascript:;" style="font-size:10px; color:#666" onClick="document.getElementById('editar_cat_<?php echo $row['id']?>').style.display=''; document.getElementById('editar2_cat_<?php echo $row['id']?>').style.display='none'" title="Editar"> <img src="imagens/edit.png" width="16" height="16"> editar </a> </div>
...
?>
        <form id="formulario_lancamento" method="post" action="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>">
          ...
          &nbsp;  |  &nbsp; <strong>Tipo:</strong>
          <label for="tipo_receita" style="color:rgba(4, 45, 191, 1)">
            <input type="radio" name="tipo" value="1" id="tipo_receita" />
            Receita</label>
          &nbsp;
          <label for="tipo_despesa" style="color:#C00">
            <input type="radio" name="tipo" value="0" checked id="tipo_despesa" />
            Despesa</label>
...
?>
          ...
          <br />
          <br />
          <font color="#000" size=1>Obs.: Em gastos parcelados, deve ser informado o valor total da compra, <br>
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;e n&atilde;o o valor da parcela.</font><br />
          <strong>Valor:</strong> R$
          <input type=text name=valor id="valor" length=15 onKeyPress="return(FormataReais(this,'.',',',event))">
  ...
  <tr>
    <td align="left" valign="top" width="450" style="background-color:#D3FFE2" class="welll"><?php
$qr=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=1 && conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$row=mysql_fetch_array($qr);
$entradas=$row['total'];
...
?>
      <fieldset>
        ...
        <table cellpadding="0" cellspacing="0" width="100%">
          ...
          <tr>
            <td><br>
              <span style="font-size:18px; color:#000">Sa&iacute;das:</span></td>
            <td align="right"><br>
              <span style="font-size:18px; color:#C00"><?php echo formata_dinheiro($saidas) ?></span></td>
          </tr>
          <tr>
    ...
    <td width="15"></td>
    <td align="left" valign="top" width="450" style="background-color:#F1F1F1" class="welll"><fieldset>
        <legend>Balan&ccedil;o Geral</legend>
        <?php
$qr=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=1 && conta=1 && usuario='$usuario'");
$row=mysql_fetch_array($qr);
$entradas=$row['total'];

$qr=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=0 && conta=1 && usuario='$usuario'");
$row=mysql_fetch_array($qr);
$saidas=$row['total'];
...
?>
        <table cellpadding="0" cellspacing="0" width="100%">
          ...
          <tr>
            <td><br>
              <span style="font-size:19px; color:#000">Sa&iacute;das:</span></td>
            <td align="right"><br>
              <span style="font-size:16px; color:#C00"><?php echo formata_dinheiro($saidas)?></span></td>
          </tr>
          <tr>
            <td colspan="2"><hr size="1" /></td>
          </tr>
...
<table cellpadding="5" cellspacing="1" width="1000" align="center">
  <tr>
    <td><a href="javascript:;" style="font-size:17px; color:#035D81" onClick="abreFecha('export_pdf');" title="Relatórios PDF"><img src="imagens/pdf.png" width="48" height="48"> </a></td>
    <td align="right"><?php
$qrconta2=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=0 && conta=2 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$row=mysql_fetch_array($qrconta2);
$conta2=$row['total'];

$qrconta3=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=0 && conta=3 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
...
<table border="0" cellpadding="5" cellspacing="1" width="1000" align="center">
  <tr style="display:none; background-color:#FFFFFF" id="export_pdf" >
    <td style="font-size:14px"  class="welll"><b>
      <center>
        Movimentos mensal</b><br>
        Informe m&ecirc;s e ano desejados.
      </center>
      <br>
      <br>
      <form method="post" action="./exportar/smovimentos.php">
        <input type="hidden" name="acao" value="movimentos" />
        <input type="hidden" name="conta" value="1" />
        <input type="hidden" name="nome" value="Movimento de Servi&ccedil;os" />
        M&ecirc;s:
        <input type="number" name="mes" size="2" maxlength="2" value="<?php echo date('m')?>" />
        <br>
        <br>
        Ano:
        <input type="number" name="ano" size="4" maxlength="4" value="<?php echo date('Y')?>" />
        <br>
        <br>
        <center>
    ...
    <td style="font-size:14px"  class="welll"><b>
      ...
      <br>
      <br>
      <form method="post" action="./exportar/sestatistica.php">
    ...
    <td style="font-size:14px"  class="welll"><b>
      <center>
        Exclus&otilde;es de movimentos</b><br>
        Listar exclus&otilde;es desta conta.
      </center>
      <form method="post" action="./exportar/sexclusoes.php">
        <input type="hidden" name="conta" value="1" />
        <input type="hidden" name="nome" value="Movimento de Servi&ccedil;os" />
        <br>
        <br>
...
<table cellpadding="5" cellspacing="0" width="1000" align="center">
  <tr>
    <td colspan="2"><div style="float:right; text-align:right" class="welll">
        <form name="form_filtro_cat" method="get" action=""  >
          ...
          <b><img src="imagens/filtro.png" width="16" height="16"> Filtrar por categoria:</b>
          <select name="filtro_cat" onChange="form_filtro_cat.submit()">
            <option value="">Tudo</option>
            <?php
$qr=mysql_query("SELECT DISTINCT c.id, c.nome, c.usuario FROM scategorias c, smovimentos m WHERE m.cat=c.id && c.usuario='$usuario' && m.mes=$mes_hoje && m.ano=$ano_hoje && m.conta=1 ORDER BY c.nome");
while ($row=mysql_fetch_array($qr)){
?>
      ...
      <h2>Movimentos deste m&ecirc;s </h2></td>
  <tr>
...
if (isset($_GET['filtro_cat'])){
	if ($_GET['filtro_cat']!=''){	
		$filtros="&& cat='".$_GET['filtro_cat']."'";
                ...
                $qr=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=1 && conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
                $row=mysql_fetch_array($qr);
                $entradas=$row['total'];

                $qr=mysql_query("SELECT SUM(valor) as total FROM smovimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
                $row=mysql_fetch_array($qr);
                $saidas=$row['total'];
...
$qr=mysql_query("SELECT * FROM smovimentos WHERE conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje' $filtros ORDER BY dia desc");
$cont=0;
while ($row=mysql_fetch_array($qr)){
$cont++;

$cat=$row['cat'];
$qr2=mysql_query("SELECT nome FROM scategorias WHERE id='$cat'");
  ...
  <tr style="display:none; background-color:<?php if ($cont%2==0) echo "#F1F1F1"; else echo "#E0E0E0"?>" id="editar_mov_<?php echo $row['id']?>">
    <td colspan="3" ><hr/>
      <form method="post" action="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>">
        ...
        <input type="hidden" name="id" value="<?php echo $row['id']?>" />
        <b>Dia:</b>
        <input type="text" name="dia" size="2" maxlength="2" value="<?php echo $row['dia']?>" />
        &nbsp;|&nbsp; <b>M&ecirc;s:</b>
        <input type="text" name="mes" size="2" maxlength="2" value="<?php echo $row['mes']?>" />
        &nbsp;|&nbsp; <b>Ano:</b>
        <input type="text" name="ano" size="3" maxlength="4" value="<?php echo $row['ano']?>" />
        &nbsp;|&nbsp; <b>Tipo:</b>
        <label for="tipo_receita<?php echo $row['id']?>" style="color:rgba(4, 45, 191, 1)">
          <input <?php if($row['tipo']==1) echo "checked=checked"?> type="radio" name="tipo" value="1" id="tipo_receita<?php echo $row['id']?>" />
          Receita</label>
        &nbsp;
        <label for="tipo_despesa<?php echo $row['id']?>" style="color:#C00">
          <input <?php if($row['tipo']==0) echo "checked=checked"?> type="radio" name="tipo" value="0" id="tipo_despesa<?php echo $row['id']?>" />
          Despesa</label>
        &nbsp;&nbsp;&nbsp;|&nbsp; <b>Categoria:</b>
        <select name="cat">
          <?php
$qr2=mysql_query("SELECT * FROM scategorias where usuario='$usuario' ORDER BY nome");
...
?>
        ...
        <br />
        <b>Valor:</b> R$
        <input type=text id="<?php echo $row['id']?>" value="<?php echo $row['valor']?>" name=valor length=15 onKeyPress="return(FormataReais(this,'.',',',event))">
        </select>
        &nbsp;&nbsp;&nbsp;&nbsp; | &nbsp;
        <input type="submit" class="btn-success" value="Gravar" />
      </form>
      <div style="text-align: right"> <a href="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>&acao=apagar&id=<?php echo $row['id']?>" title="Remover" class="btn-warning" style="color:#FF0000" onClick="return confirm('Tem certeza que deseja apagar?')">[remover]</a> </div>
      <hr/></td>
  </tr>
  <tr style="display:none; background-color:<?php if ($cont%2==0) echo "#F1F1F1"; else echo "#E0E0E0"?>" id="hist_mov_<?php echo $row['id']?>">
    ...
    <td><?php
$id=$row['id'];
$hist=mysql_query("SELECT * FROM shistorico WHERE id_mov = '$id' ORDER BY id_hist");
$qrhist=mysql_query("SELECT j.just, h.data, h.id_hist FROM (sjust_ed j INNER JOIN shistorico h ON j.id = h.just_id) INNER JOIN smovimentos g ON h.id_mov = g.id && g.id = '$id' ORDER BY h.id_hist");
...
?>
  <tr>
    <td colspan="3" align="right" class="welll"><strong style="font-size:22px; color:<?php if ($resultado_mes<0) echo "#C00"; else echo "rgba(4, 45, 191, 1)"?>"><?php echo formata_dinheiro($resultado_mes)?></strong></td>
  </tr>
</table>
</div>
<table cellpadding="5" cellspacing="0" width="1000" align="center">
