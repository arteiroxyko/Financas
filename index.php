...
if (isset($_GET['acao']) && $_GET['acao'] == 'apagar') {
	...
	$qr1=mysql_query( "SELECT * FROM movimentos WHERE comp_img='$id_comp_img'");
	$row1=mysql_fetch_array($qr1);
	if (empty($row1)){
		mysql_query( "DELETE FROM comprovantes WHERE id='$id_comp_img'");
	}
...
exit();
    ...
    if ($dia != $rowed['dia']) {
        ...
        mysql_query( "INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','1','$dataed','1','$usuario')");
        echo mysql_error($_SG['conexao']);
    }

    if ($mes != $rowed['mes']) {
        mysql_query( "UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
        mysql_query( "INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','2','$dataed','1','$usuario')");
        echo mysql_error($_SG['conexao']);
    }

    if ($ano != $rowed['ano']) {
        mysql_query( "UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
        mysql_query( "INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','3','$dataed','1','$usuario')");
        echo mysql_error($_SG['conexao']);
    }

    if ($tipo != $rowed['tipo']) {
        mysql_query( "UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
        mysql_query( "INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','4','$dataed','1','$usuario')");
        echo mysql_error($_SG['conexao']);
    }

    if ($cat != $rowed['cat']) {
        mysql_query( "UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
        mysql_query( "INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','5','$dataed','1','$usuario')");
        echo mysql_error($_SG['conexao']);
    }

    if ($descricao != $rowed['descricao']) {
        mysql_query( "UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
        mysql_query( "INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','6','$dataed','1','$usuario')");
        echo mysql_error($_SG['conexao']);
    }

    if ($valor != $rowed['valor']) {
        mysql_query( "UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
        mysql_query( "INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','7','$dataed','1','$usuario')");
        echo mysql_error($_SG['conexao']);
    }
    ...
    if (!empty($file_tmp)){
		...
		if (empty($comp_cad)){
			...
			fclose($fp);
			mysql_query( "INSERT INTO comprovantes (comp, nome, tipo, ext, tamanho) values ('$filename','$nome','$file_type','$extensao','$tamanhoemMB')");
			unlink($caminho);
			
			$dados=mysql_query( "SELECT * FROM comprovantes WHERE id=(SELECT MAX(id) FROM comprovantes)");
			$dados2=mysql_fetch_array($dados);
			$id_img=$dados2['id'];
			mysql_query( "UPDATE movimentos SET edicao='Editado', comp_img='$id_img' WHERE id='$id'");
			mysql_query( "INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','9','$dataed','$conta_lan','$usuario')");
		...
		$filename=addslashes($filename);
		fclose($fp);
		mysql_query( "UPDATE comprovantes SET comp='$filename', nome='$nome', tipo='$file_type', ext='$extensao', tamanho='$tamanhoemMB' WHERE id='$comp_cad'");
		unlink($caminho);
		mysql_query( "UPDATE movimentos SET edicao='Editado' WHERE id='$id'");
		mysql_query( "INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','9','$dataed','$conta_lan','$usuario')");
...
exit;
	...
	$dados2=mysql_fetch_array($dados);
	$id_img=$dados2['id'];
    mysql_query( "INSERT INTO movimentos (dia,mes,ano,tipo,descricao,valor,cat,conta,nparcela,parcelas,usuario,comp_img) values ('$dia','$mes','$ano','$tipo','$descricao','$valor','$cat','1','$n','$parcelas','$usuario','$id_img')");
	...
	while ($n <= $parcelas) {
    mysql_query( "INSERT INTO movimentos (dia,mes,ano,tipo,descricao,valor,cat,conta,nparcela,parcelas,usuario) values ('$dia','$mes','$ano','$tipo','$descricao','$valor','$cat','1','$n','$parcelas','$usuario')");
...
alert('O campo VALOR é obrigatório, e precisa ser diferente de zero.'); location.href='index.php'; historico.go(-1);
    ...
    if ($tipo != 0) {
        mysql_query( "UPDATE orcamento SET valor='$valor_orcamento' WHERE mes='$mes' && conta='1' && usuario='$usuario'");
        echo mysql_error($_SG['conexao']);
        header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano']);
        exit();
    }
    mysql_query( "UPDATE orcamento SET valor='$valor_orcamento' WHERE mes>=$mes && ano='$ano' && conta='1' && usuario='$usuario'");
    echo mysql_error($_SG['conexao']);
    header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano']);
    exit();
}
...
<td width="15%"><b><font color="#000" size=1><?php echo "Acesso Nº: " ?><?php if ($n = 0)
    echo "1";
else
    echo "$n_acesso" ?></font></td>
<td width="53%"><font color="#000"><?php echo $mensagem ?><?php echo " " ?><?php echo $_SESSION['usuarioNome']; ?><?php echo
" " ?><?php echo
...
<?php
$qr = mysql_query( "SELECT id, nome FROM categorias where usuario='$usuario' ORDER BY nome"); 
if (mysql_num_rows($qr) == 0) echo "Adicione ao menos uma categoria";
...
<?php
$qr = mysql_query( "SELECT SUM(valor) as total FROM movimentos WHERE tipo=1 && conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$row = mysql_fetch_array($qr);
$entradas = $row['total'];

$qr = mysql_query( "SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$row = mysql_fetch_array($qr);
$saidas = $row['total'];
...
<?php
$qr = mysql_query( "SELECT SUM(valor) as total FROM movimentos WHERE tipo=1 && conta=1 && usuario='$usuario'");
$row = mysql_fetch_array($qr);
$entradas = $row['total'];

$qr = mysql_query( "SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=1 && usuario='$usuario'");
$row = mysql_fetch_array($qr);
$saidas = $row['total'];
...
<?php
$qr = mysql_query( "SELECT DISTINCT c.id, c.nome, c.usuario FROM categorias c, movimentos m WHERE m.cat=c.id && c.usuario='$usuario' && m.mes=$mes_hoje && m.ano=$ano_hoje && m.conta=1 ORDER BY c.nome");
while ($row = mysql_fetch_array($qr)) {
?>
...
$qr = mysql_query("SELECT * FROM movimentos WHERE conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje' $filtros ORDER BY dia");
$cont = 0;
while ($row = mysql_fetch_array($qr)) {
$cont++;
